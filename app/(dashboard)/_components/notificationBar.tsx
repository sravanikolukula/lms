...
const NotificationBar: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchNotifications = async () => {
      try {
        const response = await axios.get(`/api/user/notifications?userId=${user.id}`);
        setNotifications(response.data.Notifications || []);
      } catch (error) {
        console.error("Error fetching notifications:", error);
      }
  ...
  // Redirect only in the client (after render)
  useEffect(() => {
    if (user === null) {
      router.push("/"); // ✅ Safe client-side navigation
    }
  }, [user, router]);
        ...
        >
          ...
          <DropdownMenuGroup>
            {notifications.map((notification) => {
              ...
              return (
                <Link
                  href={`/notifications/${notification.id}?${queryString}`}
                  key={notification.id}
                  passHref
                >
                  <DropdownMenuItem className="flex space-x-2">
                    <Image
                      src={notification.teacherImage}
                      alt="teacher"
                      className="rounded-full"
                      height={40}
                      width={40}
                    ...
                    <div className="flex flex-col text-left">
                      <div className="flex items-center space-x-1">
                        <h6 className="text-md font-medium truncate max-w-[210px]">
                          {notification.courseName}
                        </h6>
                        <p className="text-xs text-slate-500 md:min-w-[115px] text-right">
                          {notification.timeAgo}
                        </p>
                      </div>
                      <p className="text-sm font-light truncate max-w-[280px]">
